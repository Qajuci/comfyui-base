...
ltdrdata/ComfyUI-Impact-Pack|IMPACTPACK_SHA
"
...
fetch_latest_sha() {
  ...
  response=$(git ls-remote https://github.com/${repo} HEAD) || { echo "ERROR"; return; }
  echo "$response" | sed -n 's/\([a-f0-9]\{40\}\).*/\1/p' | head -1
}
